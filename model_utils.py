...
def bin_score_count(score_df, bins, label):
    # 等频分箱--score
    score_bin  = pd.qcut(score_df, bins).to_frame().reset_index().rename(columns={'score':'bin'}).drop('index',axis=1)
    score_bin = pd.concat([score_bin, label], axis=1)

    # 所有客户统计
    result = score_bin.bin.value_counts().to_frame().reset_index().rename(columns={'index':'ValueRange',
                                                                                   'bin':'Cnt'}).sort_values(by='ValueRange')
    # 好客户
    good_label = score_bin[score_bin.y==0].bin.value_counts().to_frame().reset_index().rename(columns={'index':'ValueRange',
                                                                                                       'bin':'GoodCnt'}).sort_values(by='ValueRange')

    # 坏客户
    bad_label = score_bin[score_bin.y==1].bin.value_counts().to_frame().reset_index().rename(columns={'index':'ValueRange',
                                                                                                      'bin':'BadCnt'}).sort_values(by='ValueRange')
    # 好坏客户合并
    good_bad_result = pd.merge(good_label, bad_label, on=['ValueRange'])
    results = pd.merge(result, good_bad_result, on=['ValueRange'])
    results['Pct'] = results.Cnt/results.Cnt.sum()
    results['GoodPct'] = results.GoodCnt/results.GoodCnt.sum()
    results['BadPct'] = results.BadCnt/results.BadCnt.sum()
    results['CumPct'] = results.Pct.cumsum()
    results['CumGoodPct'] = results.GoodPct.cumsum()
    results['CumBadPct'] = results.BadPct.cumsum()
    return results

# 交叉表比率--纵向v，横向h
def cross_rate(v_df, h_df, bins, columns_h, inedx_v):
    v_results = []
    for i in range(bins): # lgb
        h_results = []
        for j in range(bins):  # xgb
            h_rate = round(h_df[j]/v_df[i], 4)
            h_results.append(h_rate)
        v_results.append(h_results)
    v_results = np.array(v_results)
    result_hv = pd.DataFrame(v_results)
    result_plot_hv = result_hv.copy()
    result_hv.columns = columns_h
    all_results = pd.concat([inedx_v,result_hv],axis=1)
    return all_results, result_plot_hv